trigger UpdateUserName on User (after insert, after update) { 
    ...
    if (Trigger.isAfter && Trigger.isUpdate) {
        ...
        for(Id userId:Trigger.newMap.keySet()){
            ...
            if(Trigger.oldMap.get(userId).Email != Trigger.newMap.get(userId).Email && Trigger.oldMap.get(userId).ContactId != null  || Test.isRunningTest()){
        ...
        if (userSetId.isEmpty() == False || Test.isRunningTest()) {
